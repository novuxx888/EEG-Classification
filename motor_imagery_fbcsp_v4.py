#!/usr/bin/env python3
"""
EEG Motor Imagery Classification - FBCSP Optimized + Enhanced
- FBCSP with 4 bands (delta, theta, mu, beta)
...
print("="*60)
print("EEG MOTOR IMAGERY - FBCSP Optimized + Enhanced")
print("="*60)
...
# ============================================================================
# 2. FBCSP - Filter Bank CSP (4 bands)
# ============================================================================
print("\n[2] Computing FBCSP features...")

def compute_csp_for_band(X, y, fs, n_components=2, band=(8, 13)):
...
fbcsp_features = []
for band in bands:
    csp_feat = compute_csp_for_band(X, y, fs, n_components=2, band=band)
...
def extract_asymmetry(X):
    ...
    for trial in X:
        ...
        for ch in range(4):
            ...
            trial_feats.extend([asymmetry, corr, np.log(left_power+1), np.log(right_power+1)])
        features.append(trial_feats)
    return np.array(features)
...
X_combined = np.hstack([fbcsp, band_features, asym_features, spatial_features])
