#!/usr/bin/env python3
"""
EEG Motor Imagery - v4 ATTEMPT
Trying to beat 88.9%:
1. Enhanced CSP with multiple window sizes
2. Better feature engineering with wavelets
3. Tuned classifiers
4. Balanced difficulty (60% effect, 18% suppression)
"""
...
print("="*60)
print("EEG MOTOR IMAGERY - v4 ATTEMPT")
print("="*60)
...
for trial in range(n_trials):
    ...
    for ch in range(n_channels):
        # Multi-frequency EEG with more realistic rhythms
        ...
        # Add some 1/f noise (realistic EEG)
        x_1f = np.cumsum(np.random.randn(len(t)))
        x_1f = (x_1f - x_1f.mean()) / x_1f.std() * 3
        base += x_1f
        ...
        # 60% show effect, 18% suppression (harder)
        show_effect = np.random.rand() < 0.60
        suppression = 0.82 if show_effect else 1.0
        
        if label == 0:  # LEFT - right motor cortex
            if ch in [2, 3, 6, 7]:
                base *= (0.82 + np.random.uniform(-0.10, 0.10)) * suppression
...
print(f"    Difficulty: 60% effect, 18% suppression")

# ============================================================================
# 2. ENHANCED CSP WITH MULTIPLE WINDOWS
# ============================================================================
print("\n[2] Computing Enhanced CSP features with multiple windows...")

def compute_csp_multiwindow(X, y, fs, n_components=4, band=(8, 13), windows=[(0, 4), (1, 3), (2, 4)]):
    """CSP with multiple time windows"""
    all_features = []
    ...
    for start, end in windows:
        ...
        X_filt = np.array([filtfilt(b, a, trial) for trial in X_window])
...
# 6-band FBCSP with multiple windows
bands = [
    (2, 4),    # Delta
    (4, 8),    # Theta
    (8, 13),   # Mu
...
fbcsp_features = []
for band in bands:
    csp_feat = compute_csp_multiwindow(X, y, fs, n_components=4, band=band)
...
# ============================================================================
# 3. ADDITIONAL FEATURES - WAVELETS + MORE
# ============================================================================
print("\n[3] Extracting wavelet and band features...")

def morlet_wavelet(freq, t, sigma=2):
    """Morlet wavelet"""
    x = t * freq
    return np.exp(1j * 2 * np.pi * x) * np.exp(-x**2 / (2 * sigma**2))

def extract_wavelet_features(X, fs):
    """Extract wavelet-based time-frequency features"""
    features = []
    freqs = [8, 10, 12, 15, 18, 22, 25]
    widths = [3, 3, 3, 4, 4, 5, 5]
    
    for trial in X:
        trial_feats = []
        for ch in trial:
            for f, w in zip(freqs, widths):
                # Simple wavelet convolution approximation
                x = np.linspace(-1, 1, int(fs/f*w))
                wavelet = np.exp(1j * 2 * np.pi * f * x) * np.exp(-x**2 / (2 * (w/(2*np.pi*f))**2))
                wavelet = wavelet / np.max(np.abs(wavelet))
                
                # Convolve
                conv = np.convolve(ch, wavelet.real, mode='same')
                power = np.mean(conv**2)
    ...
    for trial in X:
        ...
        for ch in trial:
            ...
            trial_feats.extend([
                ...
                theta/alpha,
                beta_low/beta_high,
                alpha/(delta + theta + 1e-10),
                (alpha + beta_low)/(theta + delta + 1e-10)
            ])
            # Time domain
            trial_feats.extend([
...
def extract_asymmetry(X):
    ...
    for trial in X:
        ...
        # Beta asymmetry
        left_beta = np.mean([np.mean(trial[ch]**2) for ch in [0,1,4,5]])
        right_beta = np.mean([np.mean(trial[ch]**2) for ch in [2,3,6,7]])
        beta_asym = (left_beta - right_beta) / (left_beta + right_beta + 1e-10)
        ...
        features.append([
            asymmetry, alpha_asym, beta_asym,
            np.log(left_power + 1), np.log(right_power + 1),
            left_power / (right_power + 1e-10),
            np.mean(corrs), np.std(corrs), np.min(corrs), np.max(corrs)
        ])
    return np.array(features)
...
def extract_spatial_features(X):
    ...
    for trial in X:
        ...
        features.append([
            ...
            np.std(channel_powers), np.mean(channel_powers)
        ])
    return np.array(features)
...
wavelet_features = extract_wavelet_features(X, fs)
...
X_combined = np.hstack([
    fbcsp, wavelet_features, band_features, asym_features, 
...
gb = GradientBoostingClassifier(
    ...
    subsample=0.85, random_state=42, min_samples_split=3
)
...
try:
    ...
    xgb_clf = xgb.XGBClassifier(
        n_estimators=700, max_depth=10, learning_rate=0.06,
...
try:
    ...
    lgb_clf = lgb.LGBMClassifier(
        n_estimators=800, max_depth=15, learning_rate=0.05,
        subsample=0.85, colsample_bytree=0.8,
        reg_alpha=0.2, reg_lambda=1.5,
...
# MLP
mlp = MLPClassifier(
    hidden_layer_sizes=(512, 256, 128, 64), max_iter=1000,
...
try:
    ...
    # EEGNet with more capacity
    model = keras.Sequential([
        layers.Conv2D(16, (1, 64), padding='same', input_shape=(n_channels, len(t), 1)),
        ...
        layers.Conv2D(32, (1, 32), padding='same'),
        layers.BatchNormalization(),
        layers.Activation('elu'),
        layers.AveragePooling2D((1, 8)),
        layers.Dropout(0.3),
        
        layers.Conv2D(64, (1, 16), padding='same'),
        layers.BatchNormalization(),
        layers.Activation('elu'),
    ...
    model.fit(X_train_c, y_train_c, epochs=120, batch_size=16,
...
# ============================================================================
print("\n" + "="*60)
print("[7] RESULTS - HARDER DATA (60% effect, 18% suppression)")
print("="*60)
...
print(f"    Previous best: 88.9%")
improvement = best_acc - 0.889
if improvement > 0:
    print(f"    ⬆️ IMPROVEMENT: +{improvement:.1%}")
else:
    print(f"    ⬇️ Dropped: {improvement:.1%}")
