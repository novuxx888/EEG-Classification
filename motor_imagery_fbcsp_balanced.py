#!/usr/bin/env python3
"""
EEG Motor Imagery Classification - FBCSP Balanced
- FBCSP features + LightGBM
- Same balanced difficulty as v3 (65% effect, 15% suppression)
- Trying to beat v3's 77%
"""
...
print("="*60)
print("EEG MOTOR IMAGERY - FBCSP Balanced (Beat v3 77%)")
print("="*60)

# ============================================================================
# 1. CREATE BALANCED SYNTHETIC DATA (like v3)
# ============================================================================
...
for trial in range(n_trials):
    ...
    for ch in range(n_channels):
        # Multi-frequency base EEG (same as v3)
        ...
        # BALANCED motor imagery effect: 65% show effect, 15% change (same as v3)
...
print(f"    Difficulty: balanced (65% effect, 15% suppression)")

# ============================================================================
# 2. FBCSP + CSP FEATURES
# ============================================================================
print("\n[2] Computing FBCSP features...")

def compute_fbcsp(X, y, fs, n_components=2, bands=None):
    """Filter Bank CSP"""
    if bands is None:
    ...
    return np.hstack(all_features) if all_features else np.zeros((len(X), 1))
...
fbcsp = compute_fbcsp(X, y, fs, n_components=2)
csp = compute_csp(X, y, fs, n_components=3)
print(f"    FBCSP: {fbcsp.shape}, CSP: {csp.shape}")
...
X_combined = np.hstack([fbcsp, csp, band_features, asym_features])
...
# Logistic Regression
lr = LogisticRegression(random_state=42, max_iter=3000, C=0.5)
...
try:
    ...
    model.fit(X_train_c, y_train_c, epochs=60, batch_size=16, 
...
ensemble_estimators = [('svm', svm), ('rf', rf), ('et', et), ('lr', lr)]
...
print(f"    v3 Best: ExtraTrees = 77%")

if best_acc > 0.77:
    print(f"    🎉 IMPROVED over v3!")
else:
    print(f"    📊 Same difficulty as v3")

# Cross-validation
print("\n    Cross-validation (5-fold):")
cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
cv_clfs = [('SVM', svm), ('RF', rf), ('ET', et), ('Ensemble', ensemble)]
if 'lgb_clf' in dir() and lgb_clf is not None:
