#!/usr/bin/env python3
"""
EEG Motor Imagery - Record Breaker
Match original difficulty + better features + optimized ensemble
"""
...
from sklearn.ensemble import RandomForestClassifier, ExtraTreesClassifier, VotingClassifier, GradientBoostingClassifier, HistGradientBoostingClassifier
...
print("="*60)
print("EEG MOTOR IMAGERY - RECORD BREAKER")
print("="*60)

# ============================================================================
# 1. CREATE SYNTHETIC DATA (Original difficulty - match 82.5%)
# ============================================================================
...
for trial in range(n_trials):
    ...
    for ch in range(n_channels):
        ...
        # Original difficulty: 60% effect, 14% suppression
...
print(f"    Difficulty: 60% effect, 14% suppression (matches original)")

# ============================================================================
# 2. CSP + FBCSP FEATURES (Optimized)
# ============================================================================
...
def extract_connectivity(X):
    ...
    for trial in X:
        ...
        # Motor cortex pairs (C3-C4 is most important)
        c3_c4 = corr[1, 5] if n_channels > 5 else 0
        trial_feats.append(c3_c4)
        # Mean correlation per channel
        for ch in range(n_channels):
            trial_feats.append(np.mean(np.abs(corr[ch, :])))
        features.append(trial_feats)
    return np.array(features)
...
connectivity_features = extract_connectivity(X)
...
# HistGradientBoosting (scikit-learn's fast implementation)
...
results['HistGradientBoosting'] = accuracy_score(y_test, hgb.predict(X_test_s))
...
# ============================================================================
print("\n" + "="*60)
print("[7] RESULTS - RECORD BREAKER")
print("="*60)
