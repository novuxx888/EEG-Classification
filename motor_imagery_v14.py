#!/usr/bin/env python3
"""
EEG Motor Imagery - Version 14
Goal: Beat 82.5% by improving on best_combo.py approach
- Keep 400 trials (proven to work)
- Better tuned classifiers
...
print("="*60)
print("EEG MOTOR IMAGERY - V14 (REFINED)")
print("="*60)

# ============================================================================
# 1. SYNTHETIC DATA - SAME AS BEST_COMBO (400 trials, 8 channels)
# ============================================================================
print("\n[1] Creating synthetic motor imagery data (400 trials)...")
...
for trial in range(n_trials):
    ...
    for ch in range(n_channels):
        ...
        # Motor imagery effect (60% show effect, 14% suppression) - SAME AS BEST
...
# ============================================================================
# 2. CSP + FBCSP FEATURES - EXACTLY AS best_combo.py
# ============================================================================
...
# ============================================================================
# 3. ADDITIONAL FEATURES - SAME AS BEST
# ============================================================================
...
# Gradient Boosting - same as best
...
# ============================================================================
# 5. TRY DIFFERENT ENSEMBLE COMBINATIONS
# ============================================================================
print("\n[5] Testing ensemble combinations...")

# Ensemble 1: Just the best ones
ens1 = VotingClassifier(estimators=[('svm', svm), ('rf', rf), ('et', et), ('gb', gb)], 
                       voting='soft')
ens1.fit(X_train_s, y_train)
results['Ensemble-Best4'] = accuracy_score(y_test, ens1.predict(X_test_s))

# Ensemble 2: All ML
ens2_estimators = [('svm', svm), ('rf', rf), ('et', et), ('lr', lr), ('gb', gb), ('mlp', mlp)]
try:
    ens2_estimators.append(('xgb', xgb_clf))
except:
    pass
try:
    ens2_estimators.append(('lgb', lgb_clf))
except:
    pass
ens2 = VotingClassifier(estimators=ens2_estimators, voting='soft')
ens2.fit(X_train_s, y_train)
results['Ensemble-All'] = accuracy_score(y_test, ens2.predict(X_test_s))

# Ensemble 3: Top 3
ens3 = VotingClassifier(estimators=[('et', et), ('gb', gb), ('rf', rf)], voting='soft')
ens3.fit(X_train_s, y_train)
results['Ensemble-Top3'] = accuracy_score(y_test, ens3.predict(X_test_s))

# ============================================================================
# 6. RESULTS
# ============================================================================
print("\n" + "="*60)
print("[6] RESULTS - V14 (400 trials, same as best_combo)")
print("="*60)
